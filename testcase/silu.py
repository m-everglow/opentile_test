"""End-to-end launcher for the production Mojo SiLU Triton kernel.
...
def silu_forward(x: torch.Tensor) -> torch.Tensor:
    """Compile and execute the production `_silu_fwd_kernel` on OpenTile."""
    if x.dtype != torch.bfloat16:
        raise TypeError(f"SiLU case requires torch.bfloat16, got {x.dtype}")
