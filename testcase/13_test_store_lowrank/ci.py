...
TOL = {torch.float32: 1e-5, torch.bfloat16: 5e-3, torch.float16: 1e-3}

# Match original mojo test shape matrix:
#   shapes_label_cache = [(256, 1, 512, 128), (256, 8, 512, 128)]
#   shapes_key_lr      = [(1, 128), (8, 128)]
...
@pytest.mark.parametrize("label_cache_shape,key_lr_shape", SHAPES)
@pytest.mark.parametrize("kv_len", KV_LENS)
@pytest.mark.parametrize("dtype", DTYPES)
def test_store(label_cache_shape, key_lr_shape, kv_len, dtype):
    gen = torch.Generator(device="cpu").manual_seed(TEST_SEED)

    slot_mapping = torch.randperm(kv_len, generator=gen)[:kv_len]
    token_num = slot_mapping.shape[0]

    label_cache = torch.zeros(label_cache_shape, dtype=dtype)
    key_lr = torch.randn((token_num, *key_lr_shape), generator=gen, dtype=dtype)
    ...
    out = store_label_cache(
        label_cache.to(device), key_lr.to(device),
        block_idxs.to(device), token_idxs.to(device)
    )
    torch.npu.synchronize()

    tol = TOL[dtype]
    torch.testing.assert_close(
        out.cpu().float(), ref.float(), atol=tol, rtol=tol,
    )
    print(f"PASS | {label_cache_shape} kv={kv_len} {dtype} seed={TEST_SEED}")


if __name__ == "__main__":
    # test_store((256, 1, 512, 128), (1, 128), 16, torch.float32)
    pass
