...
def get_executor_info(executor):
    ...
    def format_arg(arg):
        if isinstance(arg, torch.Tensor):
            ...
            return "{" + ", ".join(f"{format_arg(k)}: {format_arg(v)}" for k, v in arg.items()) + "}"
        elif hasattr(arg, '__name__') and callable(arg):
            return arg.__name__
        elif arg is None:
            return "None"
        else:
    ...
    if len(matches) != 1:
        if len(matches) == 0:
            func_name = executor.__name__ if hasattr(executor, '__name__') else "<Unknown>"
        else:
            func_name = matches[0]
    else:
        func_name = matches[0]
...
def device_perf_npu(executor, profiling_dir="./npu_profiling", active=5):
    ...
    executor()   # warmup autotune if exists to avoid nested profiler bug
    ...
    try:
        ...
        csv_file_path = os.path.join(kernel_profiling_path, "ASCEND_PROFILER_OUTPUT", "kernel_details.csv")
    ...
    with open(csv_file_path, mode="r", newline="", encoding="utf-8") as file:
        ...
        for row in reader:
            if not row["Name"].startswith("aclnn"):
                total_durations += float(row["Duration(us)"])
                count += 1
    total_avg_time_us = total_durations / count
    return total_avg_time_us, kernel_profiling_path
