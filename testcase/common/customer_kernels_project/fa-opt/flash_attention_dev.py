...
try:
    from .gen_fa_mask import gen_fa_mask_kernel
    #from .fast_fa import _attn_fwd
except ImportError:
    from gen_fa_mask import gen_fa_mask_kernel
    #from fast_fa import _attn_fwd
...
@triton.jit
def fwd_kernel(
        q_ptr, k_ptr, v_ptr, o_ptr, l_ptr, mask_tensor_ptr,
    ...
    for block_idx in range(start_block, end_block, step):
        ...
        start_b = (task_hz_idx // q_head).to(tl.int64)
        ...
        if start_m * BLOCK_M < q_len:
            ...
            if begin.to(tl.int64) < k_len.to(tl.int64):
                ...
                mask_block_ptr = tl.make_block_ptr(
                     base=mask_tensor_ptr + start_b * ORI_MAX_Q_LEN * ORI_MAX_K_LEN,
                     shape=(q_len, k_len),
...
def fused_fwd_kernel(
    ...
    MAX_Q_LEN: tl.constexpr,
    MAX_K_LEN: tl.constexpr,
    TOTAL_Q_LEN: tl.constexpr,
    TOTAL_K_LEN: tl.constexpr,
    BATCH_SIZE: tl.constexpr,
):
    if TOTAL_Q_LEN == TOTAL_K_LEN:
        _attn_fwd(
            q_ptr, k_ptr, v_ptr, mask_tensor_ptr, l_ptr, o_ptr, 
            cu_seqlens_q, cu_seqlens_k,
            MAX_Q_LEN, MAX_K_LEN,
            scale,
            mask_stride_q, mask_stride_k,
...
class FlashAttentionFuncBAK(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, q_attn_arg, k_attn_arg,
        ...
        mask_tensor = torch.empty((bs, max_seqlen_q, max_seqlen_k), dtype=torch.bool, device=device) #.fill_(False)
        # print(f"=====> {max_seqlen_q=}, {max_seqlen_k=}, {bs=}, {mask_tensor.shape=}")
...
class FlashAttentionFunc(torch.autograd.Function):
    ...
    @staticmethod
    def forward_bak(ctx, q, k, v, q_attn_arg, k_attn_arg,
                cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, scale,
                mask_fn, sparse_opt, use_fused_bwd_qkv=True):
        bs = cu_seqlens_q.shape[0] - 1
        mask_tensor = torch.empty((bs, max_seqlen_q, max_seqlen_k), dtype=torch.bool, device=device) #.fill_(False)
...
def test_case_load_data():
    data = torch.load("/home/huawei0424/rjw/qianchuan_huawei_main/fa_input_lm_512.pt")
    q = data["q"]
    k = data["k"]
    v = data["v"]
    q_attn_arg = data["q_attn_arg"]
    k_attn_arg = data["k_attn_arg"]
    cu_seqlens_q = data["q_cumsum"]
    cu_seqlens_k = data["k_cumsum"]
    max_seqlen_q = 3000 # data["q_max"]
    max_seqlen_k = 3000 # data["k_max"]
    scale = data["scale"]
...
def test_main():
    ...
    _dir = 'fa_v6'
    # 291526, 8, 64
    num_heads = [1, 8, 8,8,16,16,8,4]
    head_dims = [1, 64, 64,64,32,32,64,128]
    seqs = [2048, 1138, 2432,320,32,64,32,118]
    batch_size = 1
    for i in range(1):
        num_head = num_heads[i]
        head_dim = head_dims[i]
        q_seq_list = [0] + [seqs[i]] * batch_size # 0 for cumsum
        k_seq_list = [0] + [seqs[i]] * batch_size  # 0 for cumsum
...
def test_case():
    ...
    # data = torch.load("/home/huawei0424/rjw/qianchuan_huawei_main/triton_ops/kernels/fa_input_lm.pt")
    data = torch.load("/home/huawei0424/rjw/qianchuan_huawei_main/triton_ops/kernels/fa_input_seq2048.pt")
    ...
    cu_seqlens_q = data["cu_seqlen_q"].to(torch.int32)
    cu_seqlens_k = data["cu_seqlen_k"].to(torch.int32)
    max_seqlen_q = data["max_seqlen_q"]  # 2689
...
def test_varlen_case():  # 10288.268555 (2689) -> 自适应2399：10.6ms  -> (cann：11.1ms) -> mask load/vf: 
    # input params
    dtype = torch.bfloat16
    DEVICE = torch.device("npu")
    ...
    max_seqlen_q = 2656 # 2689 # 2656/2900
    max_seqlen_k = 2656
    # qkv
