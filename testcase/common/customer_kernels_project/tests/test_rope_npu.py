...
def test_get_last_loc_kernel():
    ...
    atol = 1e-3
    # 计算最大绝对误差，用于调试
    max_abs_error = torch.abs(unpad_rope - v).max().item()
    # 断言校验，失败时抛出明确的错误信息
    assert torch.allclose(unpad_rope, v, rtol=rtol, atol=atol), \
        f"❌ unpad_rope 和 v 数值不匹配！rtol={rtol}, atol={atol}, 最大绝对误差={max_abs_error:.6f}"
    # 断言通过后打印成功提示和最大误差（便于手动调试）
    print(f"✅ unpad_rope 和 v 数值匹配！rtol={rtol}, atol={atol}, 最大绝对误差={max_abs_error:.6f}")
