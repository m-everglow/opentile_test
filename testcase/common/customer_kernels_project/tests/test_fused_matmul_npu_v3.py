...
def fused_matmul_fwd_kernel(
    ...
    xm_offs = (offset_xm[:, None] * K + offset_k[None, :])
    wn_offs = (offset_k[:, None] * N + offset_wn[None, :])

    accumulator = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float32)
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        x_mask = xm_offs < M * K
        w_mask = wn_offs < K * N
        x = tl.load(x_ptr + xm_offs, mask=x_mask, other=0.0)
        w = tl.load(w_ptr + wn_offs, mask=w_mask, other=0.0)
        accumulator = tl.dot(x, w, accumulator)
        # Advance the ptrs to the next K block.
        xm_offs += BLOCK_SIZE_K
        wn_offs += BLOCK_SIZE_K * N
    if HAS_BIAS:
