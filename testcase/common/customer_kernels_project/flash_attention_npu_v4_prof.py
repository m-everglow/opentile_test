...
def mask_fn(q_attn_arg, k_attn_arg, q_offset, k_offset, TYPE: tl.constexpr):
    ...
    if TYPE == 2:
        tril_causal = (q_offset[:, None] >= k_offset[None, :])
        return ((tril_causal & ((q_attn_arg[:, None] == k_attn_arg[None, :]) | (k_attn_arg[None, :] == 0))) | (q_offset[:, None] == k_offset[None, :]))
            ...
            if begin.to(tl.int64) < k_len.to(tl.int64):
                ...
                for start_n in range(begin, end, BLOCK_N):
                    ...
                    # if not SPARSE_OPT:
                        ...
                        s = s * qk_scale + tl.where(boundary_mask & mask, 0, -2**30)
...
class FlashAttentionFunc(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, q_attn_arg, k_attn_arg, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, scale, mask_fn, sparse_opt):
        ...
        BLOCK_M = 64
        BLOCK_N = 128
        # grid = (triton.cdiv(max_seqlen_q, BLOCK_M), q_head, batch_size)
        
        NUM_CORES = AICORE_NUM
        NUM_BLOCKS_M = triton.cdiv(max_seqlen_q, BLOCK_M)  # 非对齐跳过
    ...
    def backward(ctx, do):
        ...
        if ctx.v_dim > 64:
            ...
            BLOCK_M = 256
        grid = (triton.cdiv(ctx.max_seqlen_q, BLOCK_M), ctx.q_head, ctx.batch_size)
        bwd_preprocess[grid](
            o, do, d,
            cu_seqlens_q,
            ctx.q_head,
        ...
        BLOCK_M = 64
        BLOCK_N = 64
        grid = (triton.cdiv(ctx.max_seqlen_k, BLOCK_N), ctx.q_head, ctx.batch_size)
        bwd_kv_kernel[grid](
            q, k, v, dk, dv, do, l, d,
            q_attn_arg, k_attn_arg,
            cu_seqlens_q, cu_seqlens_k,
...
if __name__ == "__main__":
    ...
    q_attn_arg = torch.zeros(q_len, dtype=torch.int32, device=DEVICE)
    k_attn_arg = torch.zeros(k_len, dtype=torch.int32, device=DEVICE)
