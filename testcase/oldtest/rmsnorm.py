...
try:
    ...
    def compute_default_tiling_strategy(*args, **kwargs):
        return ((2048,),)
...
def rms_norm_forward(X, W, eps, offset, casting_mode):
    ...
    f"Y_ptr={Y.data_ptr()}",
    f"X_ptr={X.data_ptr()}",
    f"W_ptr={W.data_ptr() if W is not None else None}",
    f"RSTD_ptr={RSTD.data_ptr()}",
    f"Y_stride0={Y.stride(0)}",
    f"X_stride0={X.stride(0)}",
    f"RSTD_stride0={RSTD.stride(0)}",
    f"eps={eps}",
    f"offset={offset}",
    flush=True,
    )
