...
def reject_sampler(
    ...
    output_accept_lens = torch.empty(
...
def test_reject(batch_size, vocab_size, spec_step):
    ...
    ref_ids, ref_lens = reject_sampler_ref(
        target_probs, draft_tokens, draft_probs, rng_seed
    )
    ttx_ids, ttx_lens = reject_sampler(
        target_probs, draft_tokens, draft_probs, rng_seed
    )
    ...
    assert torch.equal(ref_lens, ttx_lens), f"len mismatch"
