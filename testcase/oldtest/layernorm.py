"""Triton/OpenTile LayerNorm inference kernel."""
...
@triton.jit
def layernorm_kernel(x, y, weight, bias, n_rows, n_cols, eps, BLOCK_N: tl.constexpr, BLOCK_M: tl.constexpr):
    ...
    for task in range(pid, tasks, programs):
        ...
        for start in range(0, n_cols, BLOCK_N):
            ...
            mask = row_mask[:, None] & (cols[None, :] < n_cols)
            values = tl.load(x + rows[:, None] * n_cols + cols[None, :], mask=mask, other=0.0).to(tl.float32)
            centered = tl.where(mask, values - mean[:, None], 0.0)
            square_sum += tl.sum(centered * centered, axis=1)
        ...
        for start in range(0, n_cols, BLOCK_N):
            ...
            b = tl.load(bias + cols, mask=cols < n_cols, other=0.0).to(tl.float32)
            out = (values - mean[:, None]) * rstd[:, None] * w[None, :] + b[None, :]
            tl.store(y + rows[:, None] * n_cols + cols[None, :], out, mask=mask)


def layernorm(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, eps: float) -> torch.Tensor:
    ...
    layernorm_kernel[(int(props["num_vectorcore"]),)](
        x, y, weight, bias, rows, cols, eps, BLOCK_N=block_n, BLOCK_M=block_m
    )
    return y
